...
abstract class ProgressIndicator extends StatefulWidget {
  ...
  /// is null then the current theme's [ColorScheme.primaryBlue] will
  /// be used by default.
  /// {@endtemplate}
  final Color? color;
  ...
  /// is also null then it defaults to the current theme's [ColorScheme.primaryBlue].
...
class _LinearProgressIndicatorPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    double getEffectiveTrackGapFraction(
        double currentValue, double trackGapFraction) {
    ...
    final double? effectiveValue =
        value == null ? null : clampDouble(value!, 0.0, 1.0);
    ...
    if (effectiveValue != null) {
      ...
      if (effectiveValue > 0) {
        drawLinearIndicator(
            startFraction: 0, endFraction: effectiveValue, color: valueColor);
      }
    ...
    if (firstLineHead < 1 - trackGapFraction) {
      final double trackStartFraction = firstLineHead > 0
          ...
          : 0;
      drawLinearIndicator(
          startFraction: trackStartFraction, endFraction: 1, color: trackColor);
    }
    ...
    if (firstLineTail > trackGapFraction) {
      ...
      final double trackEndFraction = firstLineTail < 1
          ? firstLineTail -
              getEffectiveTrackGapFraction(1 - firstLineTail, trackGapFraction)
          : 1;
      drawLinearIndicator(
        startFraction: trackStartFraction,
    ...
    if (secondLineTail > trackGapFraction) {
      final double trackEndFraction = secondLineTail < 1
          ? secondLineTail -
              getEffectiveTrackGapFraction(1 - secondLineTail, trackGapFraction)
          : 1;
      drawLinearIndicator(
          startFraction: 0, endFraction: trackEndFraction, color: trackColor);
    }
  }
